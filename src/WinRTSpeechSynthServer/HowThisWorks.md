...
compiler shenanigans it felt simpler to make a daemon serving the TTS requests.

The main UI spawns this daemon as a child process and they communicate using a named pipe.
If the daemon terminates (e.g. in case of an internal error) then a new one is to be spawned.
